...
/**
 * Add cards as notes. `null` results in addNotes' reply mean "duplicate" —
 ...
 * modelName 'Basic' uses Front/Back; 'Cloze' uses Text/Extra (the built-ins).
 */
export async function addCards(cards, { url, deck, tags = [], modelName = 'Basic' }, { fetchImpl = fetch } = {}) {
  if (cards.length === 0) return { added: 0, duplicates: 0, errors: [] };
  const fieldsOf = modelName === 'Cloze'
    ? (c) => ({ Text: c.front, Extra: c.back })
    : (c) => ({ Front: c.front, Back: c.back });
